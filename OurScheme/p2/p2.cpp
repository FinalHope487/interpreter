...
void check_params_count_and_type(const string &func_name, const vector<shared_ptr<Node>> &args, size_t expected_count, 
                                 const unordered_set<TokenType> &expected_types, bool is_ge);
...
void check_params_count_and_type(const string &func_name, const vector<shared_ptr<Node>> &args, size_t expected_count, const unordered_set<TokenType> &expected_types, bool is_ge = false) {
...
const unordered_map<string, Function> built_in_functions = {
    ...
    {"eqv?", Function("eqv?", [](const vector<shared_ptr<Node>> &args) -> shared_ptr<Node> {
        ...
        if (args[0]->type != args[1]->type && args[0]->type == TokenType::CONS && args[0]->type == TokenType::STRING) {
...
shared_ptr<Node> eval(const shared_ptr<Node> &node, const shared_ptr<Environment> &env, bool is_top_level) {
    ...
    if (first->type == TokenType::SYMBOL) {
        ...
        if (symbol == "define") {
            ...
            shared_ptr<Node> symbol_node = nullptr;
            shared_ptr<Node> exp_node = nullptr;
            while (cur_node != nullptr && cur_node->type == TokenType::CONS) {
                len++;
                if (len == 1) symbol_node = cur_node->car;
                else if (len == 2) exp_node = cur_node->car;
                cur_node = cur_node->cdr;
            }
            ...
            int len = 0;
            auto cur_node = node->cdr;
            shared_ptr<Node> arg = nullptr;
            while (cur_node != nullptr && cur_node->type == TokenType::CONS) {
                len++;
                if (len == 1) arg = cur_node->car;
                cur_node = cur_node->cdr;
            }
            if (len != 1) {
            ...
            int len = 0;
            auto cur_node = node->cdr;
            shared_ptr<Node> test_node = nullptr;
            shared_ptr<Node> then_node = nullptr;
            shared_ptr<Node> else_node = nullptr;
            while (cur_node != nullptr && cur_node->type == TokenType::CONS) {
                len++;
                if (len == 1) test_node = cur_node->car;
                else if (len == 2) then_node = cur_node->car;
                else if (len == 3) else_node = cur_node->car;
                cur_node = cur_node->cdr;
            }
            if (len < 2 || len > 3) {
        ...
        } else if (symbol == "cond") { // 比起if包含多個條件分支，每個條件分支中可以包含多個表達式，並回傳最後一個表達式的值
            if (!is_pure_list(node)) {
            ...
            for (size_t i = 0; i < cond_clauses.size(); i++) {
                if (!is_pure_list(cond_clauses[i])) {
                    throw runtime_error("ERROR (COND format) : " + pretty_print(node));
                }
                if (cond_clauses[i]->type == TokenType::NIL) {
                    throw runtime_error("ERROR (COND format) : " + pretty_print(node));
                }
            }
            for (size_t i = 0; i < cond_clauses.size(); i++) {
    ...
    Function f_obj = get<Function>(func_val->val);
